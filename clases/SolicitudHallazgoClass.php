...
Class SolicitudHallazgo{
	...
	function persistirSolicitudHallazgo(){
		$cq = new ConnQuery();
		$sql = "insert into solicitud_hallazgo (id_usuario, id_muro_mascota ) VALUE (?, ?)";
		$ps = $cq->prepare($sql);
	...
	public static function getSolicitudesByIdMuroMascota($idMuroMascota){
			...
			$sql = "select          u.id_usuario    id_usuario,
						...
						from solicitud_hallazgo sh
						join usuario u on u.id_usuario =  sh.id_usuario
						where sh.id_muro_mascota = ".$idMuroMascota.";";
